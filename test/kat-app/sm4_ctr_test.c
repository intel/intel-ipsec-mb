...
struct sm4_ctr_job_ctx {
        uint32_t *enc_keys;
        uint32_t *dec_keys;
};

static int
sm4_ctr_job_prepare(struct IMB_MGR *mb_mgr, struct IMB_JOB *job, const struct cipher_test *vec,
                    void *ctx)
{
        const size_t key_sched_size = IMB_SM4_KEY_SCHEDULE_ROUNDS * sizeof(uint32_t);
        struct sm4_ctr_job_ctx *job_ctx = calloc(1, sizeof(*job_ctx));
...
static void
sm4_ctr_job_cleanup(struct IMB_JOB *job, void *ctx)
{
        struct sm4_ctr_job_ctx *job_ctx = job->user_data;
...
static int
test_sm4_ctr_many(struct IMB_MGR *mb_mgr, const struct cipher_test *vec,
                  ...
                  const uint32_t num_jobs)
{
        ...
        const struct kat_cipher_job_ops ops = {
                .prepare = sm4_ctr_job_prepare,
                .cleanup = sm4_ctr_job_cleanup,
                .cipher_mode = IMB_CIPHER_SM4_CNTR,
...
static void
test_sm4_ctr_vectors(struct IMB_MGR *mb_mgr, struct test_suite_context *ctx, const int num_jobs)
{
        ...
        for (; v->msg != NULL; v++) {
                ...
                if (test_sm4_ctr_many(mb_mgr, v, IMB_DIR_ENCRYPT, IMB_ORDER_CIPHER_HASH, 0,
                                      num_jobs)) {
                ...
                if (test_sm4_ctr_many(mb_mgr, v, IMB_DIR_DECRYPT, IMB_ORDER_HASH_CIPHER, 0,
                                      num_jobs)) {
                ...
                if (test_sm4_ctr_many(mb_mgr, v, IMB_DIR_ENCRYPT, IMB_ORDER_CIPHER_HASH, 1,
                                      num_jobs)) {
                ...
                if (test_sm4_ctr_many(mb_mgr, v, IMB_DIR_DECRYPT, IMB_ORDER_HASH_CIPHER, 1,
                                      num_jobs)) {
...
sm4_ctr_test(struct IMB_MGR *mb_mgr)
{
        ...
        for (i = 0; i < test_num_jobs_size; i++)
                test_sm4_ctr_vectors(mb_mgr, &ctx, test_num_jobs[i]);
