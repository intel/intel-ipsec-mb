...
const uint8_t *
ossl_ml_dsa_key_get_seed(const ML_DSA_KEY *key)
{
        return key->seed;
}

int
ossl_ml_dsa_key_get_prov_flags(const ML_DSA_KEY *key)
{
        return key->prov_flags;
}

int
ossl_ml_dsa_set_prekey(ML_DSA_KEY *key, int flags_set, int flags_clr, const uint8_t *seed,
                       size_t seed_len, const uint8_t *sk, size_t sk_len)
{
        int ret = 0;

        if (key == NULL || key->pub_encoding != NULL || key->priv_encoding != NULL ||
            (sk != NULL && sk_len != key->params->sk_len) ||
            (seed != NULL && seed_len != ML_DSA_SEED_BYTES) || key->seed != NULL)
                return 0;
 ...
 */
int
ossl_ml_dsa_key_fetch_digests(ML_DSA_KEY *key, const char *propq)
{
 ...
 */
ML_DSA_KEY *
ossl_ml_dsa_key_new(OSSL_LIB_CTX *libctx, const char *propq, int evp_type)
{
        ML_DSA_KEY *ret;
...
int
ossl_ml_dsa_key_pub_alloc(ML_DSA_KEY *key)
{
        if (key->t1.poly != NULL)
                return 0;
...
int
ossl_ml_dsa_key_priv_alloc(ML_DSA_KEY *key)
{
 ...
 */
void
ossl_ml_dsa_key_free(ML_DSA_KEY *key)
{
        if (key == NULL)
                return;
 ...
 */
void
ossl_ml_dsa_key_reset(ML_DSA_KEY *key)
{
        /*
 ...
 */
ML_DSA_KEY *
ossl_ml_dsa_key_dup(const ML_DSA_KEY *src, int selection)
{
        ML_DSA_KEY *ret = NULL;
        ...
        if (src->pub_encoding == NULL && (src->priv_encoding != NULL || src->seed != NULL))
                return NULL;
        ...
        if (ret != NULL) {
                ...
                if ((selection & OSSL_KEYMGMT_SELECT_KEYPAIR) != 0) {
                        if (src->pub_encoding != NULL) {
                                ...
                                if ((ret->pub_encoding = OPENSSL_memdup(
                                             src->pub_encoding, src->params->pk_len)) == NULL)
                                        goto err;
                        }
                        if ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0) {
                                if (src->priv_encoding != NULL) {
                                        ...
                                        ret->priv_encoding =
                                                OPENSSL_secure_malloc(src->params->sk_len);
                                        if (ret->priv_encoding == NULL)
                                                goto err;
                                        memcpy(ret->priv_encoding, src->priv_encoding,
                                               src->params->sk_len);
                                }
                                if (src->seed != NULL) {
 ...
 */
int
ossl_ml_dsa_key_equal(const ML_DSA_KEY *key1, const ML_DSA_KEY *key2, int selection)
{
        int key_checked = 0;
 ...
 */
static int
public_from_private(const IMB_ML_DSA *self, const ML_DSA_KEY *key, EVP_MD_CTX *md_ctx, VECTOR *t1,
                    VECTOR *t0)
{
        int ret = 0;
        const ML_DSA_PARAMS *params = key->params;
        ...
        if (!self->matrix_expand_A(md_ctx, key->shake128_md, key->rho, &a_ntt))
                goto err;
        ...
        matrix_mult_vector(self, &a_ntt, &s1_ntt, &t);
        vector_ntt_inverse(self, &t);
...
ossl_ml_dsa_key_public_from_private(const IMB_ML_DSA *self, ML_DSA_KEY *key)
{
        ...
        ret = ((md_ctx = EVP_MD_CTX_new()) != NULL) &&
              ...
              && public_from_private(self, key, md_ctx, &key->t1, &t0) &&
...
ossl_ml_dsa_key_pairwise_check(const IMB_ML_DSA *self, const ML_DSA_KEY *key)
{
        ...
        if (!public_from_private(self, key, md_ctx, &t1, &t0))
                goto err;
 ...
 */
static int
keygen_internal(const IMB_ML_DSA *self, ML_DSA_KEY *out)
{
        int ret = 0;
        ...
        if (out->seed == NULL || (md_ctx = EVP_MD_CTX_new()) == NULL ||
            !ossl_ml_dsa_key_pub_alloc(out) || !ossl_ml_dsa_key_priv_alloc(out))
                goto err;
        ...
        ret = self->vector_expand_S(md_ctx, out->shake256_md, params->eta, priv_seed, &out->s1,
                                    &out->s2) &&
              public_from_private(self, out, md_ctx, &out->t1, &out->t0) &&
              ossl_ml_dsa_pk_encode(out) &&
              shake_xof(md_ctx, out->shake256_md, out->pub_encoding, out->params->pk_len, out->tr,
                        sizeof(out->tr)) &&
...
ossl_ml_dsa_generate_key(const IMB_ML_DSA *self, ML_DSA_KEY *out)
{
        ...
        if (sk == NULL) {
                ret = keygen_internal(self, out);
        } else {
                if ((ret = keygen_internal(self, out)) != 0 &&
...
int
ossl_ml_dsa_key_matches(const ML_DSA_KEY *key, int evp_type)
{
...
size_t
ossl_ml_dsa_key_get_pub_len(const ML_DSA_KEY *key)
{
        return key->params->pk_len;
}

size_t
ossl_ml_dsa_key_get_collision_strength_bits(const ML_DSA_KEY *key)
{
        return key->params->bit_strength;
}

int
ossl_ml_dsa_key_get_security_category(const ML_DSA_KEY *key)
{
...
const uint8_t *
ossl_ml_dsa_key_get_priv(const ML_DSA_KEY *key)
{
        return key->priv_encoding;
}

size_t
ossl_ml_dsa_key_get_priv_len(const ML_DSA_KEY *key)
{
        return key->params->sk_len;
}

size_t
ossl_ml_dsa_key_get_sig_len(const ML_DSA_KEY *key)
{
...
const char *
ossl_ml_dsa_key_get_name(const ML_DSA_KEY *key)
{
