 ...
 */
static void
signature_init(ML_DSA_SIG *sig, POLY *hint, uint32_t k, POLY *z, uint32_t l, uint8_t *c_tilde,
               size_t c_tilde_len)
{
...
ossl_ml_dsa_mu_init(const ML_DSA_KEY *key, int encode, const uint8_t *ctx, size_t ctx_len)
{
        if (key == NULL)
                return NULL;
        return ossl_ml_dsa_mu_init_int(key->shake256_md, key->tr, sizeof(key->tr), encode, 0, ctx,
                                       ctx_len);
}
                     ...
                     size_t mu_len, const uint8_t *rnd, size_t rnd_len, uint8_t *out_sig)
{
         ...
         */
        if (!shake_xof_3(md_ctx, priv->shake256_md, priv->K, sizeof(priv->K), rnd, rnd_len, mu,
                         mu_len, rho_prime, sizeof(rho_prime)))
                goto err;
        ...
        for (kappa = 0;; kappa += l) {
                ...
                if (!shake_xof_2(md_ctx, priv->shake256_md, mu, mu_len, w1_encoded, w1_encoded_len,
                                 c_tilde, c_tilde_len))
                        break;
                ...
                if (constant_time_declassify_u32(constant_time_ge(z_max, gamma1 - params->beta) |
                ...
                if (constant_time_declassify_u32(constant_time_ge(ct0_max, gamma2) |
                       ...
                       size_t mu_len, const uint8_t *sig_enc, size_t sig_enc_len)
{
        ...
        if (!shake_xof_3(md_ctx, pub->shake256_md, mu, mu_len, w1_encoded, w1_encoded_len, NULL, 0,
                         c_tilde, c_tilde_len))
                goto err;
