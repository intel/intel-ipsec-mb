                     ...
                     VECTOR *s2)
{
        ...
        for (i = 0; i < l; i++) {
                if (!rej_bounded_poly(h_ctx, md, coef_from_nibble_fn, derived_seed,
                                      sizeof(derived_seed), &s1->poly[i]))
                        goto err;
                ++derived_seed[ML_DSA_PRIV_SEED_BYTES];
        }
        for (i = 0; i < k; i++) {
                if (!rej_bounded_poly(h_ctx, md, coef_from_nibble_fn, derived_seed,
                                      sizeof(derived_seed), &s2->poly[i]))
                        goto err;
                ++derived_seed[ML_DSA_PRIV_SEED_BYTES];
        }
        ret = 1;
                        ...
                        uint32_t kappa, uint32_t gamma1, EVP_MD_CTX *h_ctx, const EVP_MD *md)
{
        ...
        for (i = 0; i < out->num_poly; i++) {
                ...
                poly_expand_mask(out->poly + i, derived_seed, sizeof(derived_seed), gamma1, h_ctx,
                                 md);
        }
        OPENSSL_cleanse(derived_seed, sizeof(derived_seed));
}
