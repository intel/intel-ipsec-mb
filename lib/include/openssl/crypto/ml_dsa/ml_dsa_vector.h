...
#define CONSTTIME_SECRET_VECTOR(v)     CONSTTIME_SECRET(v.poly, v.num_poly * sizeof(POLY));
#define CONSTTIME_DECLASSIFY_VECTOR(v) CONSTTIME_DECLASSIFY(v.poly, v.num_poly * sizeof(POLY));
...
vector_init(VECTOR *v, POLY *polys, size_t num_polys)
{
        v->poly = polys;
...
vector_alloc(VECTOR *v, size_t num_polys)
{
...
static ossl_inline ossl_unused int
vector_secure_alloc(VECTOR *v, size_t num_polys)
{
...
static ossl_inline ossl_unused void
vector_secure_free(VECTOR *v, size_t rank)
{
...
static ossl_inline ossl_unused void
vector_make_hint(const VECTOR *ct0, const VECTOR *cs2, const VECTOR *w, uint32_t gamma2,
                 VECTOR *out)
{
        size_t i;

        for (i = 0; i < out->num_poly; i++)
                poly_make_hint(ct0->poly + i, cs2->poly + i, w->poly + i, gamma2, out->poly + i);
}
